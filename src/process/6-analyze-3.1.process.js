 ...
 */
class ProcessAnalyze31 extends Process {
  constructor() {
    super();
    // Dependencies.
    this.env = new Env();
  ...
  process() {
    const chart = (data) => `
      ...
      <body>
        <h1>Microservices Services in Comparison with Database Technologies</h1>
        ...
        Plotly.newPlot('scatterPlot', [scatterPlot, regressionLine], layoutScatterPlot).then(() => {
          ...
          scatterPlot.on('plotly_click', function(data) {
            ...
            let counter = 0;
            ${JSON.stringify(data)}
              .filter(m => m.nbServices === x && m.nbDatabases === y)
              .forEach(m => {
                counter++;
                details.innerHTML += \`
                <p>
    ...
    const analyze = async () => {
      // ---
      // ANALYZE: microservice complexity in comparison with databases technologies / X = microservice complexity (i.e., number of services in Docker Compose file excluding databases services) ; Y = database technologies (i.e., number of services in Docker Compose file excluding non-database services).
      // ---

      // Variables.

      let data = [];
      let nbTotalServices = 0;
      let nbTotalDatabases = 0;
      ...
      while (await cursor.hasNext()) {
        ...
        nbTotalServices += nbServices;
        nbTotalDatabases += nbDatabases;
      }
      ...
      let html = path.join('results', 'analyze-31.html');
...
let process31 = new ProcessAnalyze31();
process31.process();
