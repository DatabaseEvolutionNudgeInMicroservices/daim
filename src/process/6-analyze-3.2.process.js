 ...
 */
class ProcessAnalyze32 extends Process {
  constructor() {
    super();
    // Dependencies.
    this.env = new Env();
  ...
  process() {
    const chart = (data) => `
      ...
      <body>
        <h1>Microservices Services in Comparison with Database Categories</h1>
        ...
        const xData = ${JSON.stringify(data.map((x) => x.nbServices))};
        ...
        const scatterPlot = {
          ...
          hovertemplate: '# services: %{x}, # databases categories: %{y}',
        };
        ...
        Plotly.newPlot('scatterPlot', [scatterPlot, regressionLine], layout).then(() => {
          ...
          scatterPlot.on('plotly_click', function(data) {
            ...
            let counter = 0;
            ${JSON.stringify(data)}
              .filter(m => m.nbServices === x && m.nbDatabaseCategories === y)
              .forEach(m => {
                counter++;
                details.innerHTML += \`
                <p>
                  ...
                  <ul>
                    <li># services: \${m.nbServices}</li>
                    <li># databases: \${m.nbDatabases}</li>
    ...
    const analyze = async () => {
      // ---
      // ANALYZE: microservice complexity in comparison with databases categories / X = microservice complexity (i.e., number of services in Docker Compose file excluding databases services) ; Y = database categories (i.e., number of database categories concerning services in Docker Compose file excluding non-database services).
      // ---
      ...
      const values = data.map((d) => d['nbServices']).sort((a, b) => a - b);
      const q1 = values[Math.floor(values.length * 0.25)];
      const q3 = values[Math.floor(values.length * 0.75)];
      const iqr = q3 - q1;
      const upper = q3 + 1.5 * iqr;
      data = data.filter((d) => d['nbServices'] <= upper); // Removing outliers via interquartile range.

      // HTML file.

      let html = path.join('results', 'analyze-32.html');
...
let process32 = new ProcessAnalyze32();
process32.process();
