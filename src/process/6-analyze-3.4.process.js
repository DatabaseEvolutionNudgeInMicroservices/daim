 ...
 */
class ProcessAnalyze34 extends Process {
  constructor() {
    super();
    // Dependencies.
    this.env = new Env();
  ...
  process() {
    const chart = (data) => `
      ...
      <body>
        <h1>Microservices Size in Comparison with Database Categories</h1>
        ...
        const databaseCategories = ${JSON.stringify(data.map((x) => x.databaseCategories))};
        ...
        const scatterPlot = {
          ...
          customdata: [ids, databaseCategories, databases],
          hovertemplate: 'Size: %{x}, # databases categories: %{y}',
        };
        ...
        Plotly.newPlot('scatterPlot', [scatterPlot, regressionLine], layoutScatterPlot).then(() => {
          ...
          scatterPlot.on('plotly_click', function(data) {
            ...
            let counter = 0;
            ${JSON.stringify(data)}
              .filter(m => m.size === x && m.nbDatabaseCategories === y)
              .forEach(m => {
                counter++;
                details.innerHTML += \`
                  <p>
    ...
    const analyze = async () => {
      // ---
      // ANALYZE: microservice size in comparison with databases categories / X = microservice size ; Y = database categories (i.e., number of database categories concerning services in Docker Compose file excluding non-database services).
      // ---
      ...
      let html = path.join('results', 'analyze-34.html');
...
let process34 = new ProcessAnalyze34();
process34.process();
